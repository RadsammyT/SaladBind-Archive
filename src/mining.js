...
async function continueMiner() {
	...
	fetch(`https://raw.githubusercontent.com/RadsammyT/SaladBind-Archive/${isDev ? "dev" : "main"}/internal/miners.json`)
		.then(res => res.json())
		.then(async data => {
...
async function selectPool(minerData, algo) {
	...
	fetch(`https://raw.githubusercontent.com/RadsammyT/SaladBind-Archive/${isDev ? "dev" : "main"}/internal/pools.json`)
		.then(res => res.json())
		.then(async poolData => {
				spinner.stop();
				const poolList = [];
